...
public class lab3
{
	...
	public static void Usage()
	{
		System.out.println("Usage:(for exitting, input exit or quit) \nsend <dest> <kind> <log>\t" + 
				"OR:\treceive\tOR:\tdebug\tOR:\tmulticast <group_id>\tOR:\tacquire\tOR:\trelease\n");
	}
	...
	public static String parseInput(String input)
	{
		...
		(
				(
				 	temp.length == 1 && !(temp[0].equals("receive")) && !(temp[0].equals("debug")) 
				 	&& !(temp[0].equals("groups")) && !(temp[0].equals("acquire")) 
	...
	public static void main(String[] args)
	{
		if(args.length != 2)
		{
			System.out.println("Usage: $java lab3 <conf_filename> <local_name>");
			System.exit(1);
		}
		...
		dw.start();
		MutualExclusion me = new MutualExclusion(mp, args[1]);
		me.start();
		FileMonitor fm = new FileMonitor(args[0], args[1], mp, send, recv, wq, dw, me);
		fm.start();
						...
						else if(input.startsWith("multicast"))
						{
							...
							try
							{
								...
								if((status = mp.check_group(args[1], group_num)) == 0)
								{
									...
									input = br.readLine();
									MulticastMessage r_msg = new MulticastMessage(args[1], null, kind, input, wq.getClock().inc(), wq.getClock(), group_num);
